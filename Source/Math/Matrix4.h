...
public:
	...
	__forceinline Matrix4 GetInverse() const
	{
		...
		// D#C
		Vec128 D_C = VecMat2AdjMul(D, C);
		// A#B
		Vec128 A_B = VecMat2AdjMul(A, B);
		// X# = |D|A - B(D#C)
		Vec128 X_ = VecSub(VecMul(detD, A), VecMat2Mul(B, D_C));
		// W# = |A|D - C(A#B)
		Vec128 W_ = VecSub(VecMul(detA, D), VecMat2Mul(C, A_B));

		// |M| = |A|*|D| + ... (continue later)
		Vec128 detM = VecMul(detA, detD);

		// Y# = |B|C - D(A#B)#
		Vec128 Y_ = VecSub(VecMul(detB, C), VecMat2MulAdj(D, A_B));
		// Z# = |C|B - A(D#C)#
		Vec128 Z_ = VecSub(VecMul(detC, B), VecMat2MulAdj(A, D_C));
	...
	__forceinline void Internal_GetTransposed3(Vec128& outRow0, Vec128& outRow1, Vec128& outRow2) const
	{
		Vec128 t0 = VecShuffle_0101(mVec[0], mVec[1]); // 00, 01, 10, 11
		Vec128 t1 = VecShuffle_2323(mVec[0], mVec[1]); // 02, 02, 12, 13
		outRow0 = VecShuffle(t0, mVec[2], 0, 2, 0, 3); // 00, 10, 20, 23
		outRow1 = VecShuffle(t0, mVec[2], 1, 3, 1, 3); // 01, 11, 21, 23
		outRow2 = VecShuffle(t1, mVec[2], 0, 2, 2, 3); // 02, 12, 22, 23
	}
	...
	__forceinline Matrix4 GetTransformInverse() const
	{
		...
		Internal_GetTransposed3(r.mVec[0], r.mVec[1], r.mVec[2]);
		 ...
		 // (mCol[0].SizeSqr3(), mCol[1].SizeSqr3(), mCol[2].SizeSqr3(), 0)
		Vec128 sizeSqr; 
	...
	__forceinline Matrix4 GetTransformInverseTransposed3() const
	{
		...
		// transpose 3x3, we know m03 = m13 = m23 = 0
		Vec128 row0, row1, row2;
		Internal_GetTransposed3(row0, row1, row2);

		// (mCol[0].SizeSqr3(), mCol[1].SizeSqr3(), mCol[2].SizeSqr3(), 0)
		Vec128 sizeSqr;
		sizeSqr =					VecMul(row0, row0);
		sizeSqr = VecAdd(sizeSqr,	VecMul(row1, row1));
		sizeSqr = VecAdd(sizeSqr,	VecMul(row2, row2));
	...
	__forceinline Vector4_3 InverseTransformVectorNoScale(const Vector4_3& v) const
	{
		...
		// (dot3(v, mCol[0]), dot3(v, mCol[1]), dot3(v, mCol[2]), 0)
		Vec128 r;
		r =				VecMul(row0, VecSwizzle1(v.mVec, 0));
		r = VecAdd(r,	VecMul(row1, VecSwizzle1(v.mVec, 1)));
		r = VecAdd(r,	VecMul(row2, VecSwizzle1(v.mVec, 2)));
	...
	{
		return InverseTransformVectorNoScale(v - mCol[3]);
	}
	// return value W = 0
	inline Vector4_3 InverseTransformPoint(const Vector4_3& v) const
	{
		return InverseTransformVector(v - mCol[3]);
	}
